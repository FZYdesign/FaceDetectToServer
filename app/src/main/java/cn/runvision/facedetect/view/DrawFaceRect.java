...
public class DrawFaceRect extends View {
     ...
     * @param right
     * @param bottom
     * @param faceRectOffset 这个值是用来调整左右偏移的
     */
    public void setPosition(float left, float top, float right, float bottom,int faceRectOffset) {
        this.wRate = vWidth;
        this.wRate = wRate / pWidth;
        this.hRate = vHeight;
        this.hRate = hRate / pHeight;

        this.left = wRate * left + faceRectOffset;
        this.right = wRate * right + faceRectOffset;
        this.top = hRate * top;
