...
int connect_server(char *host, int port) {
    ...
    //LOGD(">>>>>>>>>>>>>>>socket_connect  sock[%d]",ctrl_sock);
    if (ctrl_sock == -1) {
        return -1;
    }
...
int ftp_pasv_connect(int c_sock) {
    ...
    if (send_re == 0) {
        sscanf(re_buf, "%*[^(](%d,%d,%d,%d,%d,%d)", &addr[0], &addr[1], &addr[2], &addr[3],
               &addr[4], &addr[5]);
    }
...
int ftp_list(int c_sock, char *path, void **data, unsigned long long *data_len) {
    ...
    char* re_buf_char = (char *) re_buf;
    while ((len = recv(d_sock, buf, 512, 0)) > 0) {
        if (total_len + len > buf_len) {
            buf_len *= 2;
            void *re_buf_n = malloc(buf_len);
            char* re_buf_n_char = (char *) re_buf_n;
            memcpy(re_buf_n_char, re_buf_char, total_len);
            free(re_buf_char);
            re_buf_char = re_buf_n_char;
        }
        memcpy(re_buf_char + total_len, buf, len);
        total_len += len;
    }
    close(d_sock);
...
int ftp_retrfile(int c_sock, char *s, char *d, unsigned long long *stor_size, int *stop) {
    ...
    //打开本地文件
    handle = open(d, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
...
int ftp_connect(char *host, int port, char *user, char *pwd) {
    ...
    //LOGD("connect_server  sock[%d]",c_sock);
